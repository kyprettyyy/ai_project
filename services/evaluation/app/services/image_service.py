...
def _build_completions_url() -> str:
    base = settings.gateway_openai_base_url
    if not base:
        raise BusinessException(ErrorCode.PARAMS_ERROR, "请配置 EvalRoute Gateway 的 GATEWAY_BASE_URL")
    if base.endswith("/v1"):
...
async def generate_images(
    ...
    """
    调用 EvalRoute Gateway 生成图片，上传 COS，可选保存到会话。
    ...
    if resp_text.lstrip().lower().startswith("<!doctype") or resp_text.lstrip().startswith("<html"):
        raise BusinessException(
            ...
            "图片服务返回了 HTML 而非 API 数据，请检查 .env 中 GATEWAY_BASE_URL 是否指向 EvalRoute Gateway（应为 https://gateway.ai/api），不要填前端地址（如 localhost:5173）。"
        )
    try:
        data = resp.json()
    except Exception as e:
        raise BusinessException(
            ErrorCode.SYSTEM_ERROR,
            f"图片服务返回无法解析为 JSON，请确认 GATEWAY_BASE_URL=https://gateway.ai/api。响应预览: {(resp_text[:120] + '...') if len(resp_text) > 120 else resp_text}"
        ) from e
