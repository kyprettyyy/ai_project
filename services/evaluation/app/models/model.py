...
class Model(Base):
    """
    模型信息表模型（存储从EvalRoute Gateway同步的模型列表）
    """
    __tablename__ = 'model'

    id = Column(String(100), primary_key=True, comment='模型ID（EvalRoute Gateway格式，如：openai/gpt-4o）')
    ...
    raw_data = Column('rawData', Text, nullable=True, comment='EvalRoute Gateway原始数据（JSON）')
