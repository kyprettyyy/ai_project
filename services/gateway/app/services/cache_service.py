...
class CacheService:
    ...
    async def get_cached_response(self, request: ChatRequest) -> ChatResponse | None:
        if not self.is_cache_enabled() or request.evaluation_run_id:
            return None
    ...
    def generate_cache_key(request: ChatRequest) -> str:
        ...
        routing_context = json.dumps(
            {"task": request.task_type, "strategy": request.routing_strategy, "weights": request.routing_weights},
            sort_keys=True,
        )
        routing_md5 = hashlib.md5(routing_context.encode("utf-8")).hexdigest()  # noqa: S324
        key = f"{CACHE_KEY_PREFIX}{model}:{routing_md5}:{messages_md5}"
